...
// import dependencies
import repository from './users.repositories';
import validation from './users.validations';
...
// import migration
import migration from '../../migrations/admin.json';

// import constant
import { ROLES, CODE, MESSAGE } from '../../libs/constant';
import { User, UserData } from '../../libs/types';
...
async function readAllUsers(query: any) {
  // deconstruct
...
async function readOneUser(id: any) {
  // check if id exist
  if (!id) {
    throw wrap.result(CODE.BAD_REQUEST, MESSAGE.BAD_REQUEST);
  }
...
async function updateUser(id: any, body: any) {
  // check if id exist
  if (!id) {
    throw wrap.result(CODE.BAD_REQUEST, MESSAGE.BAD_REQUEST);
  }
  ...
  // change field name & obscure confidential info
  const updated: UserData = { ...currentData, ...updateData };
  updated.id = id;
